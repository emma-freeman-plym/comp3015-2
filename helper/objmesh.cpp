...
std::unique_ptr<ObjMesh> ObjMesh::load(const char *fileName, bool center,
                                       bool genTangents) {
  ...
  // Load into VAO
  mesh->initBuffers(&(glMesh.faces), &glMesh.points, &glMesh.normals,
  ...
  cout << "Loaded mesh from: " << fileName
       ...
       << "    " << mesh->bbox.toString() << endl;

  return mesh;
}

std::unique_ptr<ObjMesh> ObjMesh::loadWithAdjacency(const char *fileName,
                                                    bool center) {
...
ObjMesh::ObjMeshData::ObjVertex::ObjVertex(std::string &vertString,
                                           ObjMeshData *mesh)
  ...
  if (slash1 != string::npos) {
    ...
    if (slash2 > slash1 + 1) {
      ...
      if (tcIdx < 0)
        tcIdx += mesh->texCoords.size();
      else
        tcIdx--;
    }
    nIdx = std::stoi(vertString.substr(slash2 + 1));
    if (nIdx < 0)
      nIdx += mesh->normals.size();
    else
      nIdx--;
  }
}
...
void ObjMesh::ObjMeshData::generateTangents() {
  ...
  for (GLuint i = 0; i < faces.size(); i += 3) {
    ...
    vec3 tan1((t2 * q1.x - t1 * q2.x) * r, (t2 * q1.y - t1 * q2.y) * r,
              (t2 * q1.z - t1 * q2.z) * r);
    vec3 tan2((s1 * q2.x - s2 * q1.x) * r, (s1 * q2.y - s2 * q1.y) * r,
              (s1 * q2.z - s2 * q1.z) * r);
